def set_paddle_x(paddle_x, image_width):
    bricka.paddle.left = SCREEN_SIZE[0] * (float(paddle_x) / image_width)

def launch_ball():
    if bricka.state == STATE_BALL_IN_PADDLE:
        bricka.ball_vel = [5,-5]
        bricka.state = STATE_PLAYING
...
import thread
bricka = Bricka()
thread.start_new_thread(bricka.run , ())
